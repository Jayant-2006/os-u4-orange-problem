...
// Recursive helper: builds a tree from index entries that share a given prefix.
// 'entries' is the full index entry array, 'count' is the number of entries,
// 'prefix' is the directory prefix at this level (e.g., "src/" or "" for root).
static int write_tree_level(IndexEntry *entries, int count, const char *prefix, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        const char *rel = e->path + strlen(prefix); // path relative to current level

        char *slash = strchr(rel, '/');

        if (slash == NULL) {
            // It's a file at this level — add it directly
            ...
            // It's inside a subdirectory — collect all entries for that subdir
            ...
            // Add a tree entry for the subdirectory
            TreeEntry *te = &tree.entries[tree.count++];
            te->mode = MODE_DIR;
            te->hash = sub_id;
